...
def print_results(bot_score_data):
    ...
    for score_data in bot_score_data:
        ...
        print('  ' + 'Score: ' + str(score_data.score) + ' Moves: ' +
              str(score_data.num_moves) + ' Collisions: ' + str(score_data.num_collisions))
        place += 1
...
def compute_robot_logic(walls, goal, bot):
    ...
    move_dist = []
    # calculate the manhattan distance between completion of available moves and goal
    for m, move in enumerate(moves):
        dist = calc_manhattan_dist(
            bot.calc_x + move[0], bot.calc_y + move[1], goal.x, goal.y)
        move_dist.append([m, dist])
    # sort available moves' distances from goal by distance
    move_dist.sort(key=lambda x: x[1])
    # with 45% chance execute the move that leads to least distance from goal
    if rand.random() < 0.45:
        selected_move = moves[move_dist[0][0]]

    # check if hit wall after move
    hit_wall = False
    for wall in walls:
    ...
    if hit_wall:
      # check if there are any available moves that does not hit wall
        for next_move in move_dist:
            ...
            if not hit_wall_move:
                ...
                break
        # if every move hits wall, stay put
        if not found_alternate_move:
            selected_move = (0, 0)
  # if bot reaches goal after move, terminate
  ...
  # otherwise execute move and return results
    bot.calc_x += selected_move[0]
    bot.calc_y += selected_move[1]
